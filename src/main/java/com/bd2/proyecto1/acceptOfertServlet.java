 ...
 * @author Yarman
 */
public class acceptOfertServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int isPossible = ofeDB.isMadeToMe(db.getUserLogged().getId(), Integer.parseInt(request.getParameter("oferta-accept-id")));
        if (isPossible == -1) {
        ...
        int done = ofeDB.accept(db.getUserLogged().getId(),Integer.parseInt(request.getParameter("oferta-accept-id")));
        db.closeConection();
