 ...
 * @author Yarman
 */
public class rejectOfertaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int isPossible = ofeDB.isMadeToMe(db.getUserLogged().getId(), Integer.parseInt(request.getParameter("oferta-reject-id")));
        if (isPossible == -1) {
        ...
        int done = ofeDB.reject(db.getUserLogged().getId(),Integer.parseInt(request.getParameter("oferta-reject-id")));
        db.closeConection();
